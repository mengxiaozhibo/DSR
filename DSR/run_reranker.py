...
def train(train_file, test_file, feature_size, max_time_len, itm_spar_fnum, itm_dens_fnum, profile_num, params):
    ...
    if params.model_type == 'PRM':
        ...
        model = SLMR_generator(feature_size, params.eb_dim, params.hidden_size, max_time_len, itm_spar_fnum,
                              itm_dens_fnum,
                              profile_num, max_norm=params.max_norm, rep_num=params.rep_num)
        # evaluator = CMR_evaluator(feature_size, params.eb_dim, params.hidden_size, max_time_len, itm_spar_fnum,
        #                           itm_dens_fnum,
    ...
    if not params.controllable:
        ...
        training_monitor['ndcg_l'].append(res[1][-1])
        # training_monitor['de_ndcg_l'].append(res[2][0])
        training_monitor['clicks_l'].append(res[2][-1])
        ...
        for i, s in enumerate(params.metric_scope):
            # print(s, res[0][i], res[1][i], res[2][i], res[3][i], res[4][i], res[5][i])
    ...
    for epoch in range(params.epoch_num):
        ...
        for batch_no in range(batch_num):
            ...
            if params.model_type == 'CMR_generator':
                ...
                ## np.array(data_batch[1]) 去掉usr_feature
                ...
                # auc_rewards -= np.mean(base_auc_rewards, keepdims=True, axis=-1)
                # div_rewards -= np.mean(base_div_rewards, keepdims=True, axis=-1)
                #[B, T]
                # auc_rewards = auc_rewards / (np.mean(base_auc_rewards, keepdims=True, axis=-1) + 1e-8) #相对提升 归一化
                # div_rewards = div_rewards / (np.mean(base_div_rewards, keepdims=True, axis=-1) + 1e-8) #相对提升 归一化
                
                # 如果该action的动作奖励在候选品结合中处于高提升, 则认为该动作会带来比较大的提升
                # gip_auc = auc_rewards / (np.mean(auc_rewards, keepdims=True, axis=-1) + 1e-8)
                # gip_div = div_rewards / (np.mean(div_rewards, keepdims=True, axis=-1) + 1e-8)
                gip_auc = (auc_rewards - np.mean(auc_rewards, keepdims=True, axis=-1)) / (np.std(auc_rewards, axis=-1,keepdims=True) + 1e-8)
                gip_div = (div_rewards - np.mean(div_rewards, keepdims=True, axis=-1)) / (np.std(div_rewards, axis=-1,keepdims=True) + 1e-8)
                prefer_label_auc = np.cumsum(gip_auc, axis=1) #相对提升 #[B,T]
                prefer_label_div = np.cumsum(gip_div, axis=1) #相对提升 #[B,T]
                prefer_label = np.stack([prefer_label_auc, prefer_label_div], axis=-1) #[B, N, 2]

                auc_rewards -= base_auc_rewards #[B,N]
                div_rewards -= base_div_rewards #[B,N]
                auc_rewards[auc_rewards < 0] *= params.neg_reward
                div_rewards[div_rewards < 0] *= params.neg_reward
                
                loss, auc_loss, div_loss, diff_loss, prefer_vector_label = model.train(data_batch, training_prediction_order, auc_rewards, div_rewards,
                                                       params.lr, params.l2_reg, prefer_vector, params.keep_prob, 
                                                       train_prefer=train_prefer, prefer_label=prefer_label)
                auc_train_losses_step.append(auc_loss)
                div_train_losses_step.append(div_loss)
                diff_losses_step.append(diff_loss)
            elif params.model_type == 'EGR_generator':
            ...
            if step % eval_iter_num == 0:
                ...
                div_train_losses_step = []
                ##diff loss
                diff_train_loss = sum(diff_losses_step) / len(diff_losses_step) if len(
                    diff_losses_step) else 0
                diff_losses_step = []
                ...
                if not params.controllable:
                    ...
                    print("TRAIN PREFER: %.4f | AUC LOSS TRAIN: %.4f | DIV LOSS TRAIN: %.4f | DIFF LOSS TRAIN: %.4f" % (
                        params.acc_prefer, auc_train_loss, div_train_loss, diff_train_loss))

                    for i, s in enumerate(params.metric_scope):
                        print("@%d  MAP: %.4f  NDCG: %.4f  CLICKS: %.4f  ILAD: %.4f  ERR_IA: %.4f" % (
                            s, res[0][i], res[1][i], res[2][i], res[3][i], res[4][i]))

                    # print 当前batch第一个样本不同T时刻的prefer vector
                    if params.model_type == 'SLMR_generator':
                        print("TRAIN PREFER FOR CLICK:", prefer_vector[0,:,0])#[B, T+1, 2]
                        print("TRAIN PREFER FOR DIV:", prefer_vector[0,:,1])#[B, T+1, 2]
                        print("TRAIN PREFER LABEl FOR CLICK:", prefer_vector_label[0,:,0])#[B, T+1, 2]
                        print("TRAIN PREFER LABEl FOR DIV:", prefer_vector_label[0,:,1])#[B, T+1, 2]
                        print("PREFER LABEL FOR CLICK:", prefer_label[0,:,0])#[B, T, 2]
                        print("PREFER LABEL FOR DIV:", prefer_label[0,:,1])
...
def reranker_parse_args():
    ...
    parser.add_argument('--model_type', default='CMR_generator',
                        choices=['CMR_generator', 'CMR_evaluator','SLMR_generator'],
                        type=str,
                        help='algorithm name')
    ...
    parser.add_argument('--neg_reward', default=1.0, type=float, help='coeff for neg reward')
